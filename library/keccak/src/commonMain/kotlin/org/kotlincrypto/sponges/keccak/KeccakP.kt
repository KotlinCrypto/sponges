...
public fun F1600.keccakP(numRounds: Byte = roundCount) {
    execute(
        ...
        xor = { this xor it },
        rotateLeft = { n ->
            val bc = n % Long.SIZE_BITS
            val hi = this shl bc
            val lo = this ushr (Long.SIZE_BITS - bc)
            hi or lo
        },
    )
}
...
public fun F800.keccakP(numRounds: Byte = roundCount) {
    execute(
        ...
        xor = { this xor it },
        rotateLeft = { n ->
            val bc = n % Int.SIZE_BITS
            val hi = this shl bc
            val lo = this ushr (Int.SIZE_BITS - bc)
            hi or lo
        },
    )
}
...
public fun F400.keccakP(numRounds: Byte = roundCount) {
    execute(
        ...
        xor = { this xor it },
        rotateLeft = { n ->
            val bc = n % Short.SIZE_BITS
            val m = toInt() and 0xffff
            val hi = m shl bc
            val lo = m ushr (Short.SIZE_BITS - bc)
            (hi or lo).toShort()
        },
    )
}
...
public fun F200.keccakP(numRounds: Byte = roundCount) {
    execute(
        ...
        xor = { this xor it },
        rotateLeft = { n ->
            val bc = n % Byte.SIZE_BITS
            val m = toInt() and 0xff
            val hi = m shl bc
            val lo = m ushr (Byte.SIZE_BITS - bc)
            (hi or lo).toByte()
        },
    )
}
